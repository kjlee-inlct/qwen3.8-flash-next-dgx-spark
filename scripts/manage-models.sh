...
remove:
  Deletes only a directory containing .qwen38-model-manifest.json.
  The active installation MODEL_DIR is never deleted here; use
  ./uninstall.sh --purge-model for the active model.
EOF
}
...
status = d.get("status", "-")
files = d.get("files", [])
print(f"{status}\t{repo}\t{rev}\t{len(files) if isinstance(files, list) else '-'}")
PY
}
...
discover() {
  ...
  for base in "${candidates[@]}"; do
    [[ -e "${base}" ]] || continue
    if [[ -f "${base}/.qwen38-model-manifest.json" ]]; then
      paths=("${base}")
    elif [[ -d "${base}" ]]; then
      mapfile -t paths < <(find "${base}" -mindepth 1 -maxdepth 2 -type f -name .qwen38-model-manifest.json -printf '%h\n' 2>/dev/null | sort -u)
    else
      continue
    fi
...
list_models() {
  ...
  printf '%-8s %-10s %-8s %-42s %s\n' ACTIVE STATUS SIZE REPOSITORY PATH
  found=0
