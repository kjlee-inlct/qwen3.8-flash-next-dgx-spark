# Vendored from blazux/qwen3.8-Flash-DGX commit 5be66376e8beaf96655f2d5682c82d538a970e66
...
def _copy_mamba_state_block(
    ...
    # qwen38-flash-dgx guard: valid block count per state pool + skip counter
    ...
    # Guard: a corrupt/stale block id would send the copy through a wild
    # address (observed as Xid 31 MMU faults on GB10). Skipping the copy
    # leaves one request with a stale mamba state — recoverable — instead
    # of killing the CUDA context. The counter is reported asynchronously.
    num_blocks = tl.load(state_num_blocks_ptr + state_idx)
    if dest_block_id < 0 or dest_block_id >= num_blocks:
        tl.atomic_add(guard_hits_ptr, 1)
        return
    ...
    if CONV_STATE_DIM_FIRST and is_conv_state:
        ...
        if src_block_id < 0 or src_block_id >= num_blocks:
            tl.atomic_add(guard_hits_ptr, 1)
            return
    ...
    if actual_src_block_id < 0 or actual_src_block_id >= num_blocks:
        tl.atomic_add(guard_hits_ptr, 1)
        return
...
def postprocess_mamba_fused_kernel(
    ...
    state_num_blocks_ptr,  # int64: valid block count per state pool (guard)
    guard_hits_ptr,  # int32[1]: skipped out-of-range copies (guard)
...
class MambaSpecDecodeGPUContext:
    ...
    # Guard telemetry (sync-free: pinned mirror is refreshed asynchronously
    # and read one reporting round late)
    ...
    def run_fused_precopy(
        ...
        # Guard telemetry: refresh a pinned mirror of guard_hits every 512
        # steps and report the value captured by the PREVIOUS refresh, so no
        # step ever blocks on the GPU (async-scheduling safe).
        self.guard_step += 1
