...
case "${PROFILE_CASE}" in
  ...
  hybrid-h12-ct-postload-preserve) NAME="qwen38-h12-ct-postload-preserve-v029"; IMAGE="vllm-orcarouter-v029-h12-ct-postload-preserve:v1" ;;
  ...
  hybrid-h17-ct-weight-scale2-postload) NAME="qwen38-h17-ct-weight-scale2-postload-v029"; IMAGE="vllm-orcarouter-v029-h17-ct-weight-scale2-postload:v1" ;;
...
load_source() {
  ...
  if [[ "${PROFILE_CASE}" == orcarouter || "${PROFILE_CASE}" == hybrid-h8-ct-block || "${PROFILE_CASE}" == hybrid-h9-ct-modelweight || "${PROFILE_CASE}" == hybrid-h10-ct-global-scale || "${PROFILE_CASE}" == hybrid-h11-ct-packed-modelweight || "${PROFILE_CASE}" == hybrid-h12-ct-postload-preserve || "${PROFILE_CASE}" == hybrid-h13-ct-input-scale || "${PROFILE_CASE}" == hybrid-h14-ct-input-scale-postload || "${PROFILE_CASE}" == hybrid-h15-mtp-off || "${PROFILE_CASE}" == hybrid-h16-single-seq || "${PROFILE_CASE}" == hybrid-h17-ct-weight-scale2-postload || "${PROFILE_CASE}" == hybrid-h18-ct-input-scale-lifecycle || "${PROFILE_CASE}" == hybrid-h19-ct-combined-lifecycle || "${PROFILE_CASE}" == hybrid-h20-ct-convert-diag ]]; then
    ...
    if [[ "${PROFILE_CASE}" == hybrid-h8-ct-block ]]; then
      ...
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h12-ct-postload-preserve ]]; then
      MODEL_PROFILE="hybrid-h12-ct-postload-preserve"
      SERVED_NAME="hybrid-h12-ct-postload-preserve/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h12-ct-postload-preserve-over-orcarouter"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h13-ct-input-scale ]]; then
      MODEL_PROFILE="hybrid-h13-ct-input-scale"
      SERVED_NAME="hybrid-h13-ct-input-scale/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h13-ct-input-scale-over-orcarouter"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h14-ct-input-scale-postload ]]; then
      MODEL_PROFILE="hybrid-h14-ct-input-scale-postload"
      SERVED_NAME="hybrid-h14-ct-input-scale-postload/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h14-ct-input-scale-postload-over-orcarouter"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h15-mtp-off ]]; then
      ...
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h17-ct-weight-scale2-postload ]]; then
      MODEL_PROFILE="hybrid-h17-ct-weight-scale2-postload"
      SERVED_NAME="hybrid-h17-ct-weight-scale2-postload/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h17-ct-weight-scale2-postload-over-h12"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h18-ct-input-scale-lifecycle ]]; then
      MODEL_PROFILE="hybrid-h18-ct-input-scale-lifecycle"
      SERVED_NAME="hybrid-h18-ct-input-scale-lifecycle/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h18-ct-input-scale-lifecycle-over-h12"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h19-ct-combined-lifecycle ]]; then
      MODEL_PROFILE="hybrid-h19-ct-combined-lifecycle"
      SERVED_NAME="hybrid-h19-ct-combined-lifecycle/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h19-ct-combined-lifecycle-over-h12"
      MODEL_REVISION="runtime-control"
    elif [[ "${PROFILE_CASE}" == hybrid-h20-ct-convert-diag ]]; then
      MODEL_PROFILE="hybrid-h20-ct-convert-diag"
  ...
  if [[ "${PROFILE_CASE}" == hybrid-residual || "${PROFILE_CASE}" == hybrid-group0 || "${PROFILE_CASE}" == hybrid-quant-layout || "${PROFILE_CASE}" == hybrid-h4-down || "${PROFILE_CASE}" == hybrid-h4-gate-up || "${PROFILE_CASE}" == hybrid-h4-all || "${PROFILE_CASE}" == hybrid-h5-neutral-input || "${PROFILE_CASE}" == hybrid-h6-w4a16 || "${PROFILE_CASE}" == hybrid-h7-group-metadata || "${PROFILE_CASE}" == hybrid-h20-modelopt-convert-diag ]]; then
    load_manifest || return 1
    BASE_MODEL_DIR="${MODEL_DIR}"
    BASE_MODEL_REVISION="${MODEL_REVISION}"
    if [[ "${PROFILE_CASE}" == hybrid-h20-modelopt-convert-diag ]]; then
      MODEL_PROFILE="hybrid-h20-modelopt-convert-diag"
      MODEL_DIR="${H6_W4A16_MODEL_DIR:-$HOME/models/qwen3.8-h6-modelopt-w4a16}"
      SERVED_NAME="hybrid-h20-modelopt-convert-diag/Qwen3.8-Flash-Next-Uncensored-NVFP4"
      MODEL_REPO="local/h20-modelopt-convert-diag-over-h6"
    elif [[ "${PROFILE_CASE}" == hybrid-h7-group-metadata ]]; then
...
h10_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h10-ct-global-scale ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h10" }}' 2>/dev/null || true)"
...
h12_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h12-ct-postload-preserve ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h12" }}' 2>/dev/null || true)"
  [[ "${label}" == "compressed-tensors-postload-preserve-modelweight-v1" ]]
}

h13_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h13-ct-input-scale ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h13" }}' 2>/dev/null || true)"
  [[ "${label}" == "compressed-tensors-input-scale-pertensor-v1" ]]
}
...
h17_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h17-ct-weight-scale2-postload ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h17" }}' 2>/dev/null || true)"
  [[ "${label}" == "compressed-tensors-postload-weight-scale2-lifecycle-v1" ]]
}

h18_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h18-ct-input-scale-lifecycle ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h18" }}' 2>/dev/null || true)"
  [[ "${label}" == "compressed-tensors-postload-input-scale-lifecycle-v1" ]]
}

h19_image_ok() {
  [[ "${PROFILE_CASE}" != hybrid-h19-ct-combined-lifecycle ]] && return 0
  local label
  label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h19" }}' 2>/dev/null || true)"
  [[ "${label}" == "compressed-tensors-postload-combined-lifecycle-v1" ]]
}
...
h20_image_ok() {
  ...
  case "${PROFILE_CASE}" in
    hybrid-h20-ct-convert-diag)
      ...
      [[ "${label}" == "ct-nvfp4-convert-diag-v10" ]]
      ;;
    hybrid-h20-modelopt-convert-diag)
      label="$(docker image inspect "${IMAGE}" --format '{{ index .Config.Labels "qwen38.h20" }}' 2>/dev/null || true)"
      [[ "${label}" == "modelopt-nvfp4-convert-diag-v10" ]]
      ;;
    *)
      return 0
      ;;
...
preflight() {
  ...
  if docker image inspect "${IMAGE}" >/dev/null 2>&1; then
    ...
    if [[ "${PROFILE_CASE}" == hybrid-h9-ct-modelweight ]]; then
      ...
      printf '    build with    : docker build --no-cache -t %s -f scripts/Dockerfile.v029-h17-ct-weight-scale2-postload scripts/\n' "${IMAGE}"
    elif [[ "${PROFILE_CASE}" == hybrid-h18-ct-input-scale-lifecycle ]]; then
      printf '    build with    : docker build --no-cache -t %s -f scripts/Dockerfile.v029-h18-ct-input-scale-lifecycle scripts/\n' "${IMAGE}"
    elif [[ "${PROFILE_CASE}" == hybrid-h19-ct-combined-lifecycle ]]; then
...
start_runtime() {
  ...
  h12_image_ok || { printf 'ERROR: stale or incompatible H12 image: %s; rebuild from current main\n' "${IMAGE}" >&2; exit 1; }
  h13_image_ok || { printf 'ERROR: stale or incompatible H13 image: %s; rebuild from current main\n' "${IMAGE}" >&2; exit 1; }
  h14_image_ok || { printf 'ERROR: stale or incompatible H14 image: %s; rebuild from current main\n' "${IMAGE}" >&2; exit 1; }
  h17_image_ok || { printf 'ERROR: stale or incompatible H17 image: %s; rebuild from current main\n' "${IMAGE}" >&2; exit 1; }
  ...
  extra_mount=()
  if [[ "${PROFILE_CASE}" == hybrid-h7-group-metadata || "${PROFILE_CASE}" == hybrid-h6-w4a16 || "${PROFILE_CASE}" == hybrid-h20-modelopt-convert-diag ]]; then
  ...
  if [[ "${PROFILE_CASE}" == hybrid-h20-ct-convert-diag || "${PROFILE_CASE}" == hybrid-h20-modelopt-convert-diag ]]; then
    h20_env=(
      ...
      -e QWEN38_H20_DIAG_SAMPLE_ELEMS=1024
