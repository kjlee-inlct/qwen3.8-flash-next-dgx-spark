...
STABLE_CANDIDATE_IMAGE="vllm-skinny-stable-candidate:v1"
...
Usage:
  ...
  ./scripts/runtime/orcarouter-stock-skinny.sh start STOCK|STOCK-NOSPEC|SKINNY|SKINNY-NOSPEC|SKINNY-DET|SKINNY-EXACT|STABLE-CANDIDATE
  ./scripts/runtime/orcarouter-stock-skinny.sh stop [STOCK|STOCK-NOSPEC|SKINNY|SKINNY-NOSPEC|SKINNY-DET|SKINNY-EXACT|STABLE-CANDIDATE]
...
case_values() {
  case "$1" in
    ...
    STABLE-CANDIDATE)
      IMAGE="${STABLE_CANDIDATE_IMAGE}"; CONTAINER_NAME="qwen38-orca-stable-candidate"; SPEC_VALUE=mtp; QSA_DET_VALUE=0; QSA_EXACT_VALUE=1 ;;
    *)
      printf 'ERROR: case must be STOCK, STOCK-NOSPEC, SKINNY, SKINNY-NOSPEC, SKINNY-DET, SKINNY-EXACT, or STABLE-CANDIDATE\n' >&2
      return 2
      ;;
  esac
}
...
preflight() {
  ...
  for image in "${STOCK_IMAGE}" "${SKINNY_IMAGE}" "${SKINNY_DET_IMAGE}" "${SKINNY_EXACT_IMAGE}" "${STABLE_CANDIDATE_IMAGE}"; do
    if docker image inspect "${image}" >/dev/null 2>&1; then
      ...
      elif [[ "${image}" == "${STABLE_CANDIDATE_IMAGE}" ]]; then
        printf '    build with    : docker build -t %s -f scripts/Dockerfile.stable-candidate scripts/\n' "${STABLE_CANDIDATE_IMAGE}"
      fi
      failures=1
    fi
  done
...
case "${ACTION}" in
  ...
  status)
    ...
    for id in STOCK STOCK-NOSPEC SKINNY SKINNY-NOSPEC SKINNY-DET SKINNY-EXACT STABLE-CANDIDATE; do
      case_values "${id}"
  ...
  stop)
    ...
    if [[ -n "${CASE_ID}" ]]; then
      ...
      for id in STOCK STOCK-NOSPEC SKINNY SKINNY-NOSPEC SKINNY-DET SKINNY-EXACT STABLE-CANDIDATE; do stop_one "${id}"; done
    fi
    ;;
  -h|--help|help)
    usage
